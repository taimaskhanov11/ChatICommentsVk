...
async def send_check_request(checker_user, vk_checker) -> tuple:
    tasks = []
    logger.debug(f"{checker_user} -> Отправка запросов на проверку")
    for pre_request_obj in current_posts:
        ...
        task = asyncio.create_task(vk_checker.send_request(checker_user, pre_request_obj, config.bot.check_type))
        tasks.append(task)
